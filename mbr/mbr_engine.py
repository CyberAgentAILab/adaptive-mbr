...
from utils import (
    load_dataset,
    load_matrix,  # , approx_dir, diverse_dir
...
if __name__ == "__main__":
    ...
    for filename in tqdm(filtered_files):
        ...
        if algorithm == "incremental":
            ...
            for T_budget in T_budgets:
                ...
                for itr in range(approx_iters):
                    ...
                    if approx_best in cache.keys():
                        ...
                        approx_score = compute_score(
                            df, approx_best, trg, compute_evaluate, src=src_input
                        )
            ...
            if not recompute_matrix:
                coarse_matrix = load_matrix(
                    os.path.join(matrix_dir, dataset, model_n),
                    filename,
                    coarse_sim,
                    n_samples,
                )
            ...
            if coarse_matrix is None:
                coarse_matrix = compute_score_matrix(
                    hyp, compute_coarse_sim, [src_input] * len(hyp)
                )

                coarse_matrix_filename = (
                    filename + "_" + coarse_sim + "_" + str(n_samples)
                )
                coarse_matrix_path = os.path.join(
                    matrix_dir, dataset, model_n, coarse_matrix_filename
                )
                np.savetxt(coarse_matrix_path, coarse_matrix)

            if (algorithm == "c2f") or (algorithm == "c2ff1"):
                for T_budget in T_budgets:
                    c2f_best = compute_c2f_mbr(
                        hyp=hyp,
                        matrix=matrix,
                        src=src_input,
                        T_budget=T_budget,
                        compute_coarse=compute_coarse_sim,
                        coarse_matrix=coarse_matrix,
                    )
                    c2f_score = compute_score(
                        df, c2f_best, trg, compute_evaluate, src=src_input
                    )
            ...
            for T_budget in T_budgets:
                ...
                nbys_score = compute_score(
                    df, nbys_best, trg, compute_evaluate, src=src_input
                )
